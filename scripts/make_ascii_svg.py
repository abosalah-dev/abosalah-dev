...
CONTRAST = 1.15
BRIGHTNESS = 1.0
GAMMA = 1.05          # >1 DARKENS mids -> face gains denser chars
SHARPEN = False
...
USER = "ahmed"            # shown as USER@github in the title + status bar
FULLNAME = "Ahmed Salah"  # printed after `whoami` in the status bar
...
status_text = f"{USER}@github:~$ whoami {FULLNAME}"
cursor_x = PAD + len(status_text) * 13 * 0.6 + 5   # sit just past the last glyph
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{USER}@github:~$ whoami <tspan fill="{INK}">{FULLNAME}</tspan></text>')
